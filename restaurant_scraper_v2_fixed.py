...
class RestaurantScraperV2Fixed:
    ...
    def scrape_twitter_restaurants_v2(self) -> List[Dict]:
        """API v2を使用してTwitterから飲食店開店情報を取得（レート制限対応）"""
        try:
            # API v2クライアントを作成
            ...
            for i, keyword in enumerate(self.search_keywords):
                self.logger.info(f"キーワード検索中 ({i+1}/{len(self.search_keywords)}): {keyword}")
                
                try:
                    # レート制限を避けるために待機
                    if i > 0:
                        self.logger.info("レート制限を避けるために5秒待機...")
                        time.sleep(5)
                    
                    # API v2でツイートを検索（結果数を制限）
                    response = client.search_recent_tweets(
                        query=keyword,
                        max_results=10,  # 結果数を制限
    ...
    def run_scraper(self):
        """スクレイパーを実行"""
        self.logger.info("飲食店スクレイパー開始（レート制限対応版）")
...
def main():
    """メイン関数"""
    scraper = RestaurantScraperV2Fixed()
    scraper.run_scraper()
