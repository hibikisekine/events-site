...
class ScheduledRestaurantScraper:
    ...
    def get_today_keyword(self) -> str:
        """今日のキーワードを取得（日付に基づいてローテーション）"""
        today = datetime.now()
        day_of_year = today.timetuple().tm_yday
        keyword_index = day_of_year % len(self.search_keywords)
        return self.search_keywords[keyword_index]
    ...
    def scrape_single_keyword(self, keyword: str) -> List[Dict]:
        ...
        try:
            ...
            if response.data:
                for tweet in response.data:
                    ...
                    if restaurant_info:
                        ...
                        restaurant_info['source_type'] = 'twitter_v2_scheduled'
            ...
            self.logger.info("1時間後に再実行してください")
            return []
    ...
    def run_scraper(self):
        """スクレイパーを実行"""
        self.logger.info("スケジュール実行の飲食店スクレイパー開始")
        
        # 今日のキーワードを取得
        today_keyword = self.get_today_keyword()
        self.logger.info(f"今日のキーワード: {today_keyword}")
        
        # 単一キーワードでスクレイピング
        restaurants = self.scrape_single_keyword(today_keyword)
...
def main():
    """メイン関数"""
    scraper = ScheduledRestaurantScraper()
    scraper.run_scraper()
